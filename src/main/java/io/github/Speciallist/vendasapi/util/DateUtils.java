package io.github.Speciallist.vendasapi.util;
...
public class DateUtils {
	...
	public static Date fromString(String dataString, boolean atEndOfDay) {
		if (StringUtils.hasText(dataString)) {
			var dataLocalDate = LocalDate.parse(dataString, DateTimeFormatter.ofPattern(PADRAO_FORMATACAO_DATA));
			LocalDateTime dataHora;
			if (atEndOfDay) {
				dataHora = dataLocalDate.atTime(LocalTime.of(23, 59));
			} else {
				dataHora = dataLocalDate.atStartOfDay();
			}
